...
const page = () => {
  ...
  const [mainTask,setMainTask]=useState([]);  //array
...
if(mainTask.length>0){
  ...
  return (
      ...
      <form onSubmit={submitHandler}>
        <input type='text' className='text-2xl border-zinc-800 border-2 m-5 px-4 py-2' placeholder='Enter task here' value={task} onChange={(e)=>{
        setTask(e.target.value)

        }}/>
        <input type='text' className='text-2xl border-zinc-800 border-2 m-5 px-4 py-2' placeholder='Enter description here' value={desc} onChange={(e)=>{
